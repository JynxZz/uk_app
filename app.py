...
with st.sidebar:
    ...
    with st.expander("Middle-East conflict :gun:"):
        ...
        war_ME_intensity = st.selectbox(
            key="war_ME_intensity",
            label="Intensity",
            options=("NONE", "SMALL", "MEDIUM", "LARGE", "HIGH"),
        )

    with st.expander("Pandemic :skull_and_crossbones:"):
        covid_start = st.date_input(
            key="covid_start",
            label="Start",
            min_value=datetime(2024, 1, 1, 00, 00, 00),
            max_value=datetime(2030, 12, 31, 00, 00, 00),
    ...
    event_params = {
        "covid": {"start": covid_start, "end": covid_end, "intensity": covid_intensity},
        "lockdown": {"start": lockdown_start, "end": lockdown_end},
        "war_EU": {
            "start": war_EU_start,
            "end": war_EU_end,
        ...
        "temp": {"scenario": temp_scenario},
        "elec_price": {"yoy_increase": elec_price_yoy},
    }
...
with st.container():
    ...
    if predict_go == True:
        ...
        if response.status_code == 200:
            ...
            timestamps = [
                start_date + timedelta(hours=i) for i in range(len(elect_pred))
            ]

            assert len(elect_pred) == len(
                timestamps
            ), "Lengths of predictions and timestamps do not match."
            ...
            fig1 = px.line(
                ...
                x="timestamp",
                y="temperature",
                title="Predicted temperature 2024",
            )
            fig2 = px.line(
                df_predictions,
                x="timestamp",
                y="demand",
                title="Predicted Electricity Demand for 2024",
            )
            fig3 = px.line(
                df_predictions, x="timestamp", y="pandemic", title="Pandemic intensity"
            )
